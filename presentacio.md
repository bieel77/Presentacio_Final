# Presentació de la defensa

## 1. Presentació personal

Bona tarda,

Em dic Biel Pérez i avui presentaré la meva defensa final. En aquesta presentació explicaré breument el treball realitzat, els recursos principals i els aprenentatges adquirits durant el procés.

Aquest fitxer actuarà com a fil conductor per seguir una estructura clara durant l’exposició.

---

## 2. Índex de la defensa

1. Presentació personal
2. Objectiu de la defensa
3. Enllaços als projectes
4. Documentació
5. Conclusions i aprenentatges
6. Tancament i conclusions
---

## 3. Objectiu de la defensa

L’objectiu d’aquesta defensa és presentar de manera resumida els projectes realitzats, mostrar on es poden consultar i explicar els punts més importants del procés.

Durant la presentació es destacaran:

* Els repositoris on es troben els projectes.
* La documentació utilitzada o generada.
* Les captures o referències més importants.
* Les conclusions i aprenentatges finals.

---

## 4. Enllaços als repositoris GitHub

A continuació es deixen els enllaços als repositoris dels projectes:

* **Projecte 1:** (No realitzat per Erasmus)
* **Projecte 2:** [Enllaç al repositori](https://github.com/bieel77/Projecte2_EverPia)
* **Projecte 3:** [Enllaç al repositori](https://github.com/bieel77/Projecte3_EverPia2)
* **Projecte 4:** [Enllaç al repositori](https://github.com/bieel77/Projecte4_EverPia3)
* **Projecte 5:** [Enllaç al repositori](https://github.com/bieel77/projecte5-bieel77)
* **Projecte 6:** [Enllaç al repositori](https://github.com/bieel77/projecte6-bieel77)
* **Projecte 7:** [Enllaç al repositori](https://github.com/bieel77/projecte-7-bieel77)
* **Projecte 8:** [Enllaç al repositori](https://github.com/classesSMX2n/projecte-8-bieel77)
---

## 5. Conclusions i aprenentatges

Com a conclusió, aquest treball m’ha servit per consolidar coneixements i millorar la manera d’organitzar, desenvolupar i presentar projectes.

Els principals aprenentatges han estat:

* Millorar l’ús de **Git i GitHub**.
* Aprendre a organitzar millor els fitxers i repositoris.
* Entendre la importància de documentar correctament un projecte.
* Millorar la capacitat de resoldre problemes.
* Aprendre a presentar el treball de manera clara i ordenada.

---

## 6. Tancament

Això seria tot per la meva defensa.

Moltes gràcies per la vostra atenció.
